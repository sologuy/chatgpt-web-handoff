...
| 情况 | 结果 |
|------|------|
| 文本里有档位词（`用 pro 模式` / `极速档` / `中级模式` / `极高`…） | 按文本，`effort_source=审查对象` |
| 文本里没有 | 默认 `极高`，`effort_source=默认` |
| 你显式传了 `--effort` 且文本里没档位词 | 按你传的，`effort_source=显式参数` |
| 你传的 `--effort` 与文本里的档位**不一致** | ❌ `effort_mismatch` 直接拒绝 |
...
- 审查对象会被冻结进 `intent.json` 并带哈希，`submit` 会校验它原样出现在 `request.md`
  的「目标」节里——防的是建完作业之后正文被改跑题，不是防你写。
- 上下文里有多个合理审查对象，或者选哪个会实质改变范围 → **停下来问**，不得擅自挑选。
- **附件**：用 `--attach`，可重复，最多 10 个。类型不做限制——ChatGPT 支持什么就支持
  什么（图片、PDF、代码、文档都可以）。路径在建作业时就会查实，早失败早省事。
...
编辑 `<dir>/request.md`（`$CGH new` 已渲染好骨架，job_id 与审查对象已填入）。

- 「目标」节里的审查对象**不要动**，`cgh submit` 会校验它还在不在。
- 正文里要提一句附件是什么、想让对方看什么——光传上去不说，对方未必会去读。
...
```bash
$CGH continue --from <上一轮的 job_id> -- <这轮要追问什么>
...
| `effort_mismatch` | 你传的 `--effort` 与文本里的档位对不上 | **以用户说的为准**。别自己改档位，去掉 `--effort` 让 `cgh` 自己认最省事 |
...
| `attach_not_found` / `attach_too_many` | 附件路径不存在，或超过 10 个 | 建作业时就会报，改掉重建即可，不消耗任何额度 |
| `ui_changed` + 「上传件」字样 | 附件传不上去或有清不掉的遗留上传件 | 让用户看一眼那个标签页的输入框。**不要重试**，重试会把文件重复传上去 |
