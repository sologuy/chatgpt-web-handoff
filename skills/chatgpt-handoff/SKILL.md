...
先读用户本轮原话。若调研对象没有在本轮重复写明，但当前对话上下文只指向一个明确任务，
可在 `request.md` 中补全审查对象，**无需让用户重复描述**；只有存在两个以上合理候选、
补全会实质改变调研范围时才停下来询问。
...
- 用户本轮没重复审查对象，但上下文唯一明确 → 直接建作业，并在 `request.md` 中单独写明
  「根据当前上下文补全的审查对象」及其证据边界；不要改写被冻结的用户原话。
- 上下文存在多个合理审查对象，或补全会扩大任务范围 → **停下来问**，不得擅自挑选。
...
- 「目标」节里那句用户原话**不要动**，`cgh submit` 会校验它还在不在。用户原话是审计信源，
  不要求其单独包含完整审查对象；从上下文补全的对象另起一段明确标注，禁止伪装成用户原话。
...
3. **不改写用户原话**。`--` 后面那句仍须原样保留；这不妨碍在上下文唯一明确时，
   在 `request.md` 中另行标注并补全审查对象。
